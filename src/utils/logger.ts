...
export class Logger {
    ...
    constructor(
        ...
        generalLogFilePath: string = "log.jsonl",
        errorLogFilePath: string = "error.log.jsonl"
    ) {
    ...
    public async log(message: string | null) {
        if (message) {
            ...
            if (logAccessGranted) await this.logToFile(message, LogLevel.INFO);
        }
    }

    public async error(message: string) {
        const logAccessGranted = this.logAllowed(LogLevel.ERROR);
        if (logAccessGranted) await this.logToFile(message, LogLevel.ERROR);
    }
